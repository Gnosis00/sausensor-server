...
privileged aspect AccountController_Roo_Controller {
    ...
    public String AccountController.create(@Valid Account account, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest) {
        ...
        uiModel.asMap().clear();
        ClassPathXmlApplicationContext classPathXmlApplicationContext = new ClassPathXmlApplicationContext("classpath*:/META-INF/spring/applicationContext.xml");
        ShaPasswordEncoder shaPasswordEncoder = (ShaPasswordEncoder) classPathXmlApplicationContext.getBean("passwordEncoder");
        //yarın öbürgün tek yerden herşey değişsin diye applicationContext kullandım.
        //simdilik burda dursun olmadı aop un imkanlarından yararlanırız.
        account.setPassword(shaPasswordEncoder.encodePassword(account.getPassword(), null));
        //null yerine "hash string" girebiliriz ama ben standart kullandım.
        account.persist();
        //account oluşturduk şimdi account role oluşturalım
        AccountRoles accountRoles = new AccountRoles();
        accountRoles.setAccount(account);
        accountRoles.setAUTHORITY("ROLE_USER");//user olsun standartta olmadı application context içine atarız. her yerde standart olur.
